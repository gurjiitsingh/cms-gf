...
export default function UsersByMonth() {
  ...
  const handleDelete = async (userId: string, expectedName?: string) => {
    if (!expectedName) {
      alert('No username available for confirmation.');
      return;
    }

    const input = prompt(`To confirm, type the username exactly: "${expectedName}"`);
    if (!input || input.trim() !== expectedName) {
 ...
 <div className="flex items-center justify-between mb-4">
        <h1 className="text-xl font-semibold">Users by Month</h1>
        <Link
          href="/users/by-month"
