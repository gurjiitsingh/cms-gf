...
type ExtendedAddressResT = addressResT & { _docId?: string };

export default function SearchAddress() {
  const [searchTerm, setSearchTerm] = useState("");
  const [searchBy, setSearchBy] = useState("email");
  const [results, setResults] = useState<ExtendedAddressResT[]>([]);
  ...
  const handleSearch = async () => {
    ...
    try {
      if (searchBy === "docId") {
        ...
        if (docSnap.exists()) {
          setResults([
            { ...(docSnap.data() as addressResT), _docId: docSnap.id },
          ]);
        } else {
          setNotFound(true);
        }
        ...
        if (querySnapshot.empty) {
          ...
          const data: ExtendedAddressResT[] = [];
          querySnapshot.forEach((doc) => {
            data.push({ ...(doc.data() as addressResT), _docId: doc.id });
          });
          setResults(data);
        }
      }
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-white rounded shadow space-y-6">
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-xl font-semibold">Search Address</h1>
        <Link
          href="/users/address-search"
        ...
        <div className="overflow-x-auto border rounded mt-4">
          <table className="min-w-full text-sm">
            <thead className="bg-gray-100">
              <tr>
                <th className="px-3 py-2 border">Doc ID</th>
            ...
            <tbody>
              {results.map((item, idx) => (
                <tr key={idx} className="hover:bg-gray-50">
                  <td className="px-3 py-2 border">{item._docId}</td>
