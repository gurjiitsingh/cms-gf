...
export default function TransferOrderToDummy() {
  ...
  const parseGermanTimeToTimestamp = (timeStr: string): Timestamp => {
    try {
      // Parse the `time` field as German local time
      const germanDateStr = new Date(timeStr).toLocaleString("en-DE", {
        dateStyle: "medium",
        timeStyle: "medium",
        timeZone: "Europe/Berlin",
      });

      // Convert to ISO and then to a Date object
      const parsed = new Date(germanDateStr);
  ...
  const handleTransfer = async () => {
    ...
    try {
      ...
      for (const docSnap of snapshot.docs) {
        ...
        const createdAtA = parseGermanTimeToTimestamp(data.time);

        await addDoc(collection(db, "orderMasterDummy"), {
          ...data,
          createdAt,
          createdAtA, // ✅ New field based on `time`
        });
