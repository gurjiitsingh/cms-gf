...
export default function MonthlySalesReport() {
  ...
  const fetchOrdersByMonth = async (monthStr: string) => {
    ...
    try {
      ...
      const filteredOrders: orderMasterDataT[] = snapshot.docs
        .map((doc) => ({
          id: doc.id,
          ...doc.data(),
        }))
      ...
      const total = filteredOrders.reduce(
        (sum, order) => sum + (order.endTotalG || 0),
        0
      );
      setTotalSales(total);
    } catch (error) {
      console.error('Error fetching orders by string time:', error);
    }
    setLoading(false);
  };
  ...
  return (
    <div className="p-4 max-w-6xl mx-auto">
      ...
      {loading ? (
        ...
        <p>No completed orders found for this month.</p>
      ) : (
        <>
            ...
            <tbody>
              {orders.map((order) => (
                <tr key={order.id} className="hover:bg-gray-50">
                  ...
                  <td className="border px-3 py-2">€{order.endTotalG?.toFixed(2) || '0.00'}</td>
