...
export default function TemplateList() {
  ...
  const { setTemplate, setTemplateUrl } = useAppContext();
  ...
  const handleUseTemplate = async () => {
    ...
    if (selected) {
      try {
        const tem = await fetchTemplateByUrl(selected.url);
  ...
  async function fetchTemplateByUrl(templateUrl: string) {
    try {
      const q = query(
      ...
      if (querySnapshot.empty) {
        console.log('No template found with that URL');
        return null;
      }
  ...
  return (
    <div className="p-4 md:p-6">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold">Select Template to Send Campaign</h1>
        <a
          href="/auto-campaign/template/create/select-option"
